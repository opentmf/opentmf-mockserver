            ...
            .readConfiguration(
                new ByteArrayInputStream(
                    (""
                            ...
                            + NEW_LINE
                            + "java.util.logging.SimpleFormatter.format=%1$tF %1$tT "
                            + Version.getVersion()
                            + " %4$s %5$s %6$s%n"
                            + NEW_LINE
        ...
        if (isNotBlank(ConfigurationProperties.javaLoggerLogLevel())) {
          String loggingConfiguration =
              ""
                  ...
                  + " %4$s %5$s %6$s%n"
                  + NEW_LINE
                  + "org.mockserver.level="
                  + ConfigurationProperties.javaLoggerLogLevel()
                  + NEW_LINE
                  + "io.netty.level="
                  + (Arrays.asList("TRACE", "FINEST")
                          .contains(ConfigurationProperties.javaLoggerLogLevel())
                      ? "FINE"
                      : "WARNING");
          LogManager.getLogManager()
      ...
      new MockServerLogger()
          .logEvent(
              new LogEntry()
                  ...
                  .setLogLevel(ERROR)
                  .setMessageFormat(
                      "exception while configuring Java logging - " + throwable.getMessage())
    ...
    if (!ConfigurationProperties.disableLogging()) {
      if ((logEntry.isAlwaysLog() || isEnabled(logEntry.getLogLevel()))
        ...
        switch (logEntry.getLogLevel()) {
          case ERROR:
            logger.error(
                portInformation(logEntry) + logEntry.getMessage(), logEntry.getThrowable());
            break;
          case WARN:
