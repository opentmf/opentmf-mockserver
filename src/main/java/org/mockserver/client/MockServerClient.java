...
public class MockServerClient implements Stoppable {
  ...
  private static final MockServerLogger MOCK_SERVER_LOGGER =
      new MockServerLogger(MockServerClient.class);
  ...
  private RequestDefinitionSerializer requestDefinitionSerializer =
      new RequestDefinitionSerializer(MOCK_SERVER_LOGGER);
  private ExpectationIdSerializer expectationIdSerializer =
      new ExpectationIdSerializer(MOCK_SERVER_LOGGER);
  private LogEventRequestAndResponseSerializer httpRequestResponseSerializer =
      new LogEventRequestAndResponseSerializer(MOCK_SERVER_LOGGER);
  private PortBindingSerializer portBindingSerializer =
      new PortBindingSerializer(MOCK_SERVER_LOGGER);
  private ExpectationSerializer expectationSerializer =
      new ExpectationSerializer(MOCK_SERVER_LOGGER);
  private OpenAPIExpectationSerializer openAPIExpectationSerializer =
      new OpenAPIExpectationSerializer(MOCK_SERVER_LOGGER);
  private VerificationSerializer verificationSerializer =
      new VerificationSerializer(MOCK_SERVER_LOGGER);
  private VerificationSequenceSerializer verificationSequenceSerializer =
      new VerificationSequenceSerializer(MOCK_SERVER_LOGGER);
  private final CompletableFuture<MockServerClient> stopFuture = new CompletableFuture<>();

  /**
   * Start the client communicating to a MockServer on localhost at the port specified with the
   * Future
   *
  ...
  public MockServerClient(
      ClientConfiguration configuration, CompletableFuture<Integer> portFuture) {
   ...
   *
   * <p>MockServerClient mockServerClient = new MockServerClient("localhost", 1080);
   *
  ...
  /**
   * Start the client communicating to a MockServer at the specified host and port and contextPath
   * for example:
   *
   * <p>MockServerClient mockServerClient = new MockServerClient("localhost", 1080, "/mockserver");
   *
   * @param host the host for the MockServer to communicate with
   * @param port the port for the MockServer to communicate with
   ...
   */
  public MockServerClient(
      ClientConfiguration configuration, String host, int port, String contextPath) {
    this.clientClass = MockServerClient.class;
    if (isEmpty(host)) {
      throw new IllegalArgumentException("Host can not be null or empty");
    }
  ...
  private NioEventLoopGroup eventLoopGroup() {
    return new NioEventLoopGroup(
        ...
        new Scheduler.SchedulerThreadFactory(this.getClass().getSimpleName() + "-eventLoop"));
  }
  ...
  private NettyHttpClient getNettyHttpClient() {
    if (nettyHttpClient == null) {
      NettySslContextFactory nettySslContextFactory =
          new NettySslContextFactory(
              configuration.toServerConfiguration(), MOCK_SERVER_LOGGER, false);
      Function<SslContextBuilder, SslContext> clientSslContextBuilderFunction =
          NettySslContextFactory.clientSslContextBuilderFunction;
      if (configuration.controlPlaneTLSMutualAuthenticationRequired()) {
        if (isBlank(configuration.controlPlanePrivateKeyPath())
            || isBlank(configuration.controlPlaneX509CertificatePath())
            || isBlank(configuration.controlPlaneTLSMutualAuthenticationCAChain())) {
          throw new IllegalArgumentException(
              "when 'controlPlaneTLSMutualAuthenticationRequired' is enabled 'controlPlanePrivateKeyPath', 'controlPlaneX509CertificatePath' and 'controlPlaneTLSMutualAuthenticationCAChain' must all be specified,\n\tfound controlPlanePrivateKeyPath: \""
                  + configuration.controlPlanePrivateKeyPath()
                  + "\"\n\tand controlPlaneX509CertificatePath: \""
                  + configuration.controlPlaneX509CertificatePath()
                  + "\"\n\tand controlPlaneTLSMutualAuthenticationCAChain: \""
                  + configuration.controlPlaneTLSMutualAuthenticationCAChain()
                  + "\"");
        }
            ...
            sslContextBuilder -> {
              try {
                ...
                X509Certificate[] keyCertChain =
                    x509ChainFromPEMFile(configuration.controlPlaneX509CertificatePath())
                        .toArray(new X509Certificate[0]);
                X509Certificate[] trustCertCollection =
                    nettySslContextFactory.trustCertificateChain(
                        configuration.controlPlaneTLSMutualAuthenticationCAChain());
  ...
  private HttpResponse sendRequest(
      HttpRequest request, boolean ignoreErrors, boolean throwClientException) {
    if (!stopFuture.isDone()) {
      try {
        ...
        if (controlPlaneJWTSupplier != null) {
          ...
          if (isNotBlank(jwt)) {
            ...
            throw new IllegalArgumentException(
                "Control plane jwt supplier returned invalid JWT \"" + jwt + "\"");
          }
        }
        ...
        if (response != null) {
          ...
          if (!Version.matchesMajorMinorVersion(serverVersion)) {
            throw new ClientException(
                "Client version \""
                    + clientVersion
                    + "\" major and minor versions do not match server version \""
                    + serverVersion
                    + "\"");
          }
        }
        ...
        if (throwClientException
          ...
          throw new ClientException(
              formatLogMessage("error:{}while sending request:{}", response, request));
        }

        return response;
      } catch (RuntimeException rex) {
        if (isNotBlank(rex.getMessage())
            && (rex.getMessage().contains("executor not accepting a task")
                || rex.getMessage().contains("loop shut down"))) {
          throw new IllegalStateException(
              this.getClass().getSimpleName()
                  + " has already been closed, please create new "
                  + this.getClass().getSimpleName()
                  + " instance");
        } else {
          throw rex;
      ...
      throw new IllegalStateException(
          this.getClass().getSimpleName()
              + " has already been stopped, please create new "
              + this.getClass().getSimpleName()
              + " instance");
    }
  }
  ...
  /**
   * Launch UI and wait the default period to allow the UI to launch and start collecting logs, this
   * ensures that the log are visible in the UI even if MockServer is shutdown by a test shutdown
   * function, such as After, AfterClass, AfterAll, etc
   */
  ...
  /**
   * Launch UI and wait a specified period to allow the UI to launch and start collecting logs, this
   * ensures that the log are visible in the UI even if MockServer is shutdown by a test shutdown
   * function, such as After, AfterClass, AfterAll, etc
   *
   * @param timeUnit TimeUnit the time unit, for example TimeUnit.SECONDS
   * @param pause the number of time units to delay before the function returns to ensure the UI is
   *     receiving logs
   */
    ...
    try {
          ...
          if (MockServerLogger.isEnabled(WARN)) {
            MOCK_SERVER_LOGGER.logEvent(
                new LogEntry()
                    .setLogLevel(WARN)
                    .setMessageFormat(
                        "browse to URL not supported by the desktop instance from JVM"));
          }
        }
      } else {
        if (MockServerLogger.isEnabled(WARN)) {
      ...
      MOCK_SERVER_LOGGER.logEvent(
          new LogEntry()
              .setLogLevel(ERROR)
              .setMessageFormat(
                  "exception while attempting to launch UI"
                      + (isNotBlank(throwable.getMessage()) ? " " + throwable.getMessage() : ""))
  ...
  /**
   * Returns whether MockServer is running, if called too quickly after starting MockServer this may
   * return false because MockServer has not yet started, to ensure MockServer has started use
   * hasStarted()
   *
  ...
  /**
   * Returns whether server MockServer is running, by polling the MockServer a configurable amount
   * of times. If called too quickly after starting MockServer this may return false because
   * MockServer has not yet started, to ensure MockServer has started use hasStarted()
   *
   * @deprecated use hasStopped() or hasStarted() instead
   */
  @Deprecated
  public boolean isRunning(int attempts, long timeout, TimeUnit timeUnit) {
    try {
      HttpResponse httpResponse =
          sendRequest(request().withMethod("PUT").withPath(calculatePath("status")), true, false);
      ...
      if (MockServerLogger.isEnabled(TRACE)) {
        MOCK_SERVER_LOGGER.logEvent(
            new LogEntry()
                .setLogLevel(TRACE)
                .setMessageFormat(
                    "exception while checking if MockServer is running - "
                        + sce.getMessage()
                        + " if MockServer was stopped this exception is expected")
                .setThrowable(sce));
      }
      return false;
    }
  ...
  /**
   * Returns whether MockServer has stopped, if called too quickly after starting MockServer this
   * may return false because MockServer has not yet started, to ensure MockServer has started use
   * hasStarted()
   */
  ...
  /**
   * Returns whether server MockServer has stopped, by polling the MockServer a configurable amount
   * of times. If called too quickly after starting MockServer this may return false because
   * MockServer has not yet started, to ensure MockServer has started use hasStarted()
   */
  ...
  /**
   * Returns whether MockServer has started, if called after MockServer has been stopped this method
   * will block for 5 seconds while confirming MockServer is not starting
   */
  ...
  /**
   * Returns whether server MockServer has started, by polling the MockServer a configurable amount
   * of times
   */
  public boolean hasStarted(int attempts, long timeout, TimeUnit timeUnit) {
    try {
      HttpResponse httpResponse =
          sendRequest(request().withMethod("PUT").withPath(calculatePath("status")), false);
        ...
        if (MockServerLogger.isEnabled(DEBUG)) {
          MOCK_SERVER_LOGGER.logEvent(
              new LogEntry()
                  .setLogLevel(DEBUG)
                  .setMessageFormat(
                      "exception while checking if MockServer has started - " + sce.getMessage())
                  .setThrowable(sce));
        }
        return false;
      } else {
    ...
    String boundPorts =
        sendRequest(
                request()
                    ...
                    .withBody(
                        portBindingSerializer.serialize(portBinding(ports)),
  ...
  public CompletableFuture<MockServerClient> stop(boolean ignoreFailure) {
    if (!stopFuture.isDone()) {
      ...
      new Scheduler.SchedulerThreadFactory("ClientStop")
          .newThread(
              () -> {
                try {
                  ...
                  if (!ignoreFailure && MockServerLogger.isEnabled(TRACE)) {
                    MOCK_SERVER_LOGGER.logEvent(
                        new LogEntry()
                            .setLogLevel(TRACE)
                            .setMessageFormat(
                                "request rejected while closing down, logging in case due other error "
                                    + ree)
                            .setThrowable(ree));
                  }
                } catch (Exception e) {
                  if (!ignoreFailure && MockServerLogger.isEnabled(WARN)) {
                    MOCK_SERVER_LOGGER.logEvent(
                        new LogEntry()
                            .setLogLevel(WARN)
                            .setMessageFormat(
                                "failed to send stop request to MockServer " + e.getMessage()));
                  }
                }
  ...
  public MockServerClient reset() {
    ...
    sendRequest(request().withMethod("PUT").withPath(calculatePath("reset")), true);
   ...
   *
   * @param requestDefinition the http request that is matched against when deciding whether to
   *     clear each expectation if null all expectations are cleared
   */
  ...
  public MockServerClient clear(ExpectationId expectationId) {
    sendRequest(
        request()
            ...
            .withPath(calculatePath("clear"))
            .withBody(
                expectationId != null ? expectationIdSerializer.serialize(expectationId) : "",
                StandardCharsets.UTF_8),
        true);
    return clientClass.cast(this);
  }
   ...
   * </pre>
   *
   * @param maximumNumberOfRequestToReturnInVerificationFailure the maximum number requests return
   *     in the error response when the verification fails
   * @param requestDefinitions the http requests that must be matched for this verification to pass
   * @throws AssertionError if the request has not been found
   */
      ...
      String result =
          sendRequest(
                  request()
                      ...
                      .withBody(
                          verificationSequenceSerializer.serialize(verificationSequence),
                          StandardCharsets.UTF_8),
                  false)
              .getBodyAsString();
   ...
   */
  public MockServerClient verify(
      Integer maximumNumberOfRequestToReturnInVerificationFailure, ExpectationId... expectationIds)
      throws AssertionError {
   ...
   *
   * <p>once() - verify the request was only received once exactly(n) - verify the request was only
   * received exactly n times atLeast(n) - verify the request was only received at least n times
   *
   ...
   */
  @SuppressWarnings("DuplicatedCode")
  public MockServerClient verify(RequestDefinition requestDefinition, VerificationTimes times)
      throws AssertionError {
      ...
      String result =
          sendRequest(
                  request()
                      ...
                      .withBody(
                          verificationSerializer.serialize(verification), StandardCharsets.UTF_8),
                  false)
              .getBodyAsString();
   ...
   */
  @SuppressWarnings("DuplicatedCode")
  public MockServerClient verify(String expectationId, VerificationTimes times)
      throws AssertionError {
   ...
   */
  @SuppressWarnings("DuplicatedCode")
  public MockServerClient verify(ExpectationId expectationId, VerificationTimes times)
      throws AssertionError {
  ...
  /**
   * Retrieve the recorded requests that match the httpRequest parameter, use null for the parameter
   * to retrieve all requests
   *
   * @param requestDefinition the http request that is matched against when deciding whether to
   *     return each request, use null for the parameter to retrieve for all requests
   * @return an array of all requests that have been recorded by the MockServer in the order they
   *     have been received and including duplicates where the same request has been received
   *     multiple times
   */
  public HttpRequest[] retrieveRecordedRequests(RequestDefinition requestDefinition) {
    ...
    return Arrays.stream(requestDefinitions)
  ...
  /**
   * Retrieve the recorded requests and responses that match the httpRequest parameter, use null for
   * the parameter to retrieve all requests and responses
   *
   * @param requestDefinition the http request that is matched against when deciding whether to
   *     return each request (and its corresponding response), use null for the parameter to
   *     retrieve for all requests
   * @return an array of all requests and responses that have been recorded by the MockServer in the
   *     order they have been received and including duplicates where the same request has been
   *     received multiple times
   */
  public LogEventRequestAndResponse[] retrieveRecordedRequestsAndResponses(
      RequestDefinition requestDefinition) {
   ...
   *     multiple times
   */
  public String retrieveRecordedRequestsAndResponses(
      RequestDefinition requestDefinition, Format format) {
    HttpResponse httpResponse =
        sendRequest(
            request()
                .withMethod("PUT")
  ...
  /**
   * Retrieve the request-response combinations that have been recorded as a list of expectations,
   * only those that match the httpRequest parameter are returned, use null to retrieve all requests
   *
   * @param requestDefinition the http request that is matched against when deciding whether to
   *     return each request, use null for the parameter to retrieve for all requests
   * @return an array of all expectations that have been recorded by the MockServer in the order
   *     they have been received and including duplicates where the same request has been received
   *     multiple times
   */
  ...
  /**
   * Retrieve the logs associated to a specific requests, this shows all logs for expectation
   * matching, verification, clearing, etc
   *
   * @param requestDefinition the http request that is matched against when deciding whether to
   *     return each request, use null for the parameter to retrieve for all requests
   * @return all log messages recorded by the MockServer when creating expectations, matching
   *     expectations, performing verification, clearing logs, etc
   */
   ...
   * @param requestDefinition the http request that is matched against when deciding whether to
   *     return each request, use null for the parameter to retrieve for all requests
   * @return an array of all log messages recorded by the MockServer when creating expectations,
   *     matching expectations, performing verification, clearing logs, etc
   */
   ...
   * @param requestDefinition the http request that must be matched for this expectation to respond
   * @param times the number of times to respond when this http is matched
   * @param timeToLive the length of time from when the server receives the expectation that the
   *     expectation should be active
   * @return an Expectation object that can be used to specify the response
   */
  public ForwardChainExpectation when(
      RequestDefinition requestDefinition, Times times, TimeToLive timeToLive) {
    return new ForwardChainExpectation(
        configuration,
        MOCK_SERVER_LOGGER,
        getMockServerEventBus(),
   ...
   *
   * <p>- higher priority expectation will be matched first - identical priority expectations will
   * be match in the order they were submitted - default priority is 0
   *
   * <p>Example use:
   *
   * <pre>
   ...
   * @param timeToLive the length of time from when the server receives the expectation that the
   *     expectation should be active
   * @param priority the priority for the expectation when matching, higher priority expectation
   *     will be matched first, identical priority expectations will be match in the order they were
   *     submitted
   * @return an Expectation object that can be used to specify the response
   */
  public ForwardChainExpectation when(
   ...
   *
   * @param openAPIExpectations the OpenAPI and operations and responses to create matchers and
   *     example responses
        ...
        httpResponse =
            sendRequest(
                request()
                    ...
                    .withBody(
                        openAPIExpectationSerializer.serialize(openAPIExpectations[0]),
                        StandardCharsets.UTF_8),
                false);
        if (httpResponse != null && httpResponse.getStatusCode() != 201) {
          throw new ClientException(
              formatLogMessage(
                  "error:{}while submitted OpenAPI expectation:{}",
                  httpResponse,
                  openAPIExpectations[0]));
        }
        ...
        httpResponse =
            sendRequest(
                request()
                    ...
                    .withPath(calculatePath("openapi"))
                    .withBody(
                        openAPIExpectationSerializer.serialize(openAPIExpectations),
                        StandardCharsets.UTF_8),
                true);
        if (httpResponse != null && httpResponse.getStatusCode() != 201) {
          throw new ClientException(
              formatLogMessage(
                  "error:{}while submitted OpenAPI expectations:{}",
                  httpResponse,
                  openAPIExpectations));
        }
      }
   ...
   *
   * <p>This method should be used to update existing expectation by id. All fields will be updated
   * for expectations with a matching id as the existing expectation is deleted and recreated.
   *
   * <p>To retrieve the id(s) for existing expectation(s) the retrieveActiveExpectations(HttpRequest
   * httpRequest) method can be used.
   *
   * <p>Typically, to create expectations this method should not be used directly instead the
   * when(...) and response(...) or forward(...) or error(...) methods should be used for example:
   *
   * <pre>
   * mockServerClient
   *  .when(
        ...
        httpResponse =
            sendRequest(
                request()
                    ...
                    .withBody(
                        expectationSerializer.serialize(expectations[0]), StandardCharsets.UTF_8),
                false);
        if (httpResponse != null && httpResponse.getStatusCode() != 201) {
          throw new ClientException(
              formatLogMessage(
                  "error:{}while submitted expectation:{}", httpResponse, expectations[0]));
        }
        ...
        httpResponse =
            sendRequest(
                request()
                    ...
                    .withPath(calculatePath("expectation"))
                    .withBody(
                        expectationSerializer.serialize(expectations), StandardCharsets.UTF_8),
                false);
        if (httpResponse != null && httpResponse.getStatusCode() != 201) {
          throw new ClientException(
              formatLogMessage(
                  "error:{}while submitted expectations:{}", httpResponse, expectations));
        }
      }
      if (httpResponse != null && isNotBlank(httpResponse.getBodyAsString())) {
        return expectationSerializer.deserializeArray(httpResponse.getBodyAsString(), true);
  ...
  /**
   * Specify one or more expectations, normally this method should not be used directly instead the
   * when(...) and response(...) or forward(...) or error(...) methods should be used for example:
   *
   * <pre>
   * mockServerClient
   ...
   * @deprecated this is deprecated due to unclear naming, use method upsert(Expectation...
  ...
  /**
   * Retrieve the active expectations match the httpRequest parameter, use null for the parameter to
   * retrieve all expectations
   *
   * @param requestDefinition the http request that is matched against when deciding whether to
   *     return each expectation, use null for the parameter to retrieve for all requests
