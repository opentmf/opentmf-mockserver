    ...
    try {
      ...
      FullHttpRequest request =
          new DefaultFullHttpRequest(
              HttpVersion.HTTP_1_1, httpMethod, getURI(httpRequest), getBody(httpRequest));
  ...
  public String getURI(HttpRequest httpRequest) {
    ...
    if (httpRequest.getPath() != null) {
      if (httpRequest.getQueryStringParameters() != null
        ...
        QueryStringEncoder queryStringEncoder =
            new QueryStringEncoder(httpRequest.getPath().getValue());
      ...
      for (org.mockserver.model.Cookie cookie : httpRequest.getCookieList()) {
        cookies.add(
            new io.netty.handler.codec.http.cookie.DefaultCookie(
                cookie.getName().getValue(), cookie.getValue().getValue()));
      }
      request
          .headers()
          .set(
