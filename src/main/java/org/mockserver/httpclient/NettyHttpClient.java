...
public class NettyHttpClient {
    ...
    public NettyHttpClient(Configuration configuration, MockServerLogger mockServerLogger, EventLoopGroup eventLoopGroup, boolean forwardProxyClient) {
        this(configuration, mockServerLogger, eventLoopGroup, forwardProxyClient, new NettySslContextFactory(configuration, mockServerLogger, false));
    }

    public NettyHttpClient(Configuration configuration, MockServerLogger mockServerLogger, EventLoopGroup eventLoopGroup, boolean forwardProxyClient, NettySslContextFactory nettySslContextFactory) {
    ...
    public CompletableFuture<HttpResponse> sendRequest(final HttpRequest httpRequest, @Nullable InetSocketAddress remoteAddress, Long connectionTimeoutMillis) throws SocketConnectionException {
        if (!eventLoopGroup.isShuttingDown()) {
            ...
            final HttpClientInitializer clientInitializer = new HttpClientInitializer(mockServerLogger, forwardProxyClient, nettySslContextFactory, httpProtocol);
    ...
    public CompletableFuture<BinaryMessage> sendRequest(final BinaryMessage binaryRequest, final boolean isSecure, InetSocketAddress remoteAddress, Long connectionTimeoutMillis) throws SocketConnectionException {
        if (!eventLoopGroup.isShuttingDown()) {
            ...
            new Bootstrap()
                ...
                .handler(new HttpClientInitializer(mockServerLogger, forwardProxyClient, nettySslContextFactory, null))
                .connect(remoteAddress)
                .addListener((ChannelFutureListener) future -> {
                    if (future.isSuccess()) {
