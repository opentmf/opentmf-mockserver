...
public class HttpRequest extends RequestDefinition implements HttpMessage<HttpRequest, Body> {
   ...
   * </pre>
   *
   * <p>For full details of JSON Schema see,
  ...
  /**
   * The HTTP method all method except a specific value using the "not" operator, for example this
   * allows operations such as not("GET")
   *
   ...
   * "/some_mocked_path", the servlet context path is ignored for matching and should not be
   * specified hereregex values are also supported such as not(".*_path"), see
   * http://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html for full details of the
  ...
  /**
   * The path parameter to match on as a list of Parameter objects where the values or keys of each
  ...
  /**
   * The path parameter to match on as a varags Parameter objects where the values or keys of each
   * parameter can be either a string or a regex (for more details of the supported regex syntax see
   * http://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html)
   *
  ...
  /**
   * The path parameter to match on as a Map&lt;String, List&lt;String&gt;&gt; where the values or
   * keys of each parameter can be either a string or a regex (for more details of the supported
  ...
  /**
   * Adds one path parameter to match on as a Parameter object where the parameter values list can
   * be a list of strings or regular expressions (for more details of the supported regex syntax see
   * http://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html)
   *
   ...
   * (for more details of the supported regex syntax see
   * http://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html)
   *
  ...
  /**
   * Adds one path parameter to match which the values are JSON schema i.e. "{ \"type\": \"string\",
   * \"pattern\": \"^someV[a-z]{4}$\" }" (for more details of the supported JSON schema see
  ...
  public HttpRequest withSchemaPathParameter(String name, String... values) {
    ...
    getOrCreatePathParameters()
        .withEntry(
            string(name),
            Arrays.stream(values)
                .map(NottableSchemaString::schemaString)
  ...
  /**
   * Adds one path parameter to match on or to not match on using the NottableString, each
   * NottableString can either be a positive matching value, such as string("match"), or a value to
   * not match on, such as not("do not match"), the string values passed to the NottableString can
   * also be a plain string or a regex (for more details of the supported regex syntax see
   * http://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html)
   *
  ...
  /**
   * Adds one query string parameter to match which the values are JSON schema i.e. "{ \"type\":
   * \"string\", \"pattern\": \"^someV[a-z]{4}$\" }" (for more details of the supported JSON schema
  ...
  public HttpRequest withSchemaQueryStringParameter(String name, String... values) {
    ...
    getOrCreateQueryStringParameters()
        .withEntry(
            string(name),
            Arrays.stream(values)
                .map(NottableSchemaString::schemaString)
  ...
  /**
   * Adds one query string parameter to match on or to not match on using the NottableString, each
   * NottableString can either be a positive matching value, such as string("match"), or a value to
   * not match on, such as not("do not match"), the string values passed to the NottableString can
   * also be a plain string or a regex (for more details of the supported regex syntax see
   * http://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html)
   ...
   *
   * <p>exact string match: - exact("this is an exact string body");
   *
   * <p>or
   *
   * <p>- new StringBody("this is an exact string body")
   *
   * <p>regular expression match: - regex("username[a-z]{4}");
   *
   * <p>or
   *
   * <p>- new RegexBody("username[a-z]{4}");
   *
   * <p>json match: - json("{username: 'foo', password: 'bar'}");
   *
   * <p>or
   *
   * <p>- json("{username: 'foo', password: 'bar'}", MatchType.STRICT);
   *
   * <p>or
   *
   * <p>- new JsonBody("{username: 'foo', password: 'bar'}");
   *
   * <p>json schema match: - jsonSchema("{type: 'object', properties: { 'username': { 'type':
   * 'string' }, 'password': { 'type': 'string' } }, 'required': ['username', 'password']}");
   *
   * <p>or
   *
   * <p>- jsonSchemaFromResource("org/mockserver/model/loginSchema.json");
   *
   * <p>or
   *
   * <p>- new JsonSchemaBody("{type: 'object', properties: { 'username': { 'type': 'string' },
   * 'password': { 'type': 'string' } }, 'required': ['username', 'password']}");
   *
   * <p>xpath match: - xpath("/element[key = 'some_key' and value = 'some_value']");
   *
   * <p>or
   *
   * <p>- new XPathBody("/element[key = 'some_key' and value = 'some_value']");
   *
   * <p>body parameter match: - params( param("name_one", "value_one_one", "value_one_two")
   * param("name_two", "value_two") );
   *
   * <p>or
   *
   * <p>- new ParameterBody( new Parameter("name_one", "value_one_one", "value_one_two") new
   * Parameter("name_two", "value_two") );
   *
   * <p>binary match: -
   ...
   * 1024));
   *
   * <p>for more details of the supported regular expression syntax see <a
   * href="http://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html">http://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html</a>
   * for more details of the supported json syntax see <a
   * href="http://jsonassert.skyscreamer.org">http://jsonassert.skyscreamer.org</a> for more details
   * of the supported json schema syntax see <a
   * href="http://json-schema.org/">http://json-schema.org/</a> for more detail of XPath syntax see
   * <a
   * href="http://saxon.sourceforge.net/saxon6.5.3/expressions.html">http://saxon.sourceforge.net/saxon6.5.3/expressions.html</a>
   *
   * @param body an instance of one of the Body subclasses including StringBody, ParameterBody or
   *     BinaryBody
   */
    ...
    if (body != null) {
      if (body instanceof StringBody) {
        ...
        return new String(
            body.toString()
                .getBytes(
                    MediaType.parse(getFirstHeader(CONTENT_TYPE.toString())).getCharsetOrDefault()),
  ...
  /**
   * Adds one header to match which the values are JSON schema i.e. "{ \"type\": \"string\",
   * \"pattern\": \"^someV[a-z]{4}$\" }" (for more details of the supported JSON schema see
   * https://json-schema.org)
   *
   * @param name the header name
  ...
  /**
   * Adds one header to match on or to not match on using the NottableString, each NottableString
   * can either be a positive matching value, such as string("match"), or a value to not match on,
   * such as not("do not match"), the string values passed to the NottableString can also be a plain
  ...
  /**
   * Adds one cookie to match on, which the value is plain strings or regular expressions (for more
   * details of the supported regex syntax see
   * http://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html)
   *
  ...
  /**
   * Adds one cookie to match on, which the value the values is JSON schema i.e. "{ \"type\":
   * \"string\", \"pattern\": \"^someV[a-z]{4}$\" }" (for more details of the supported JSON schema
   * see https://json-schema.org)
   *
   * @param name the cookies name
  ...
  /**
   * Adds one cookie to match on or to not match on using the NottableString, each NottableString
   * can either be a positive matching value, such as string("match"), or a value to not match on,
   * such as not("do not match"), the string values passed to the NottableString can be a plain
   * string or a regex (for more details of the supported regex syntax see
   * http://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html)
   *
   * @param name the cookies name
  ...
  public InetSocketAddress socketAddressFromHostHeader() {
    if (socketAddress != null && socketAddress.getHost() != null) {
      ...
      throw new IllegalArgumentException(
          "Host header must be provided to determine remote socket address, the request does not include the \"Host\" header:"
              + NEW_LINE
              + this);
    }
  }
  ...
  public HttpRequest update(HttpRequest requestOverride, HttpRequestModifier requestModifier) {
    if (requestOverride != null) {
      if (requestOverride.getMethod() != null
          && isNotBlank(requestOverride.getMethod().getValue())) {
    ...
    if (requestModifier != null) {
      ...
      if (requestModifier.getQueryStringParameters() != null) {
        withQueryStringParameters(
            requestModifier.getQueryStringParameters().update(getQueryStringParameters()));
      }
