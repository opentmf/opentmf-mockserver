...
public class NottableString extends ObjectWithJsonToString implements Comparable<NottableString> {
  ...
  NottableString capitalize() {
    ...
    return new NottableString(
        StringUtils.substringBeforeLast(Joiner.on("-").join(split), "_"), not);
  }
  ...
  public boolean matches(String input) {
    if (pattern == null) {
      pattern =
          Pattern.compile(
              getValue(), Pattern.DOTALL | Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
    }
