...
public class HttpRequestModifier extends ObjectWithJsonToString {
  ...
  /**
   * The regex and substitution values to use to modify matching substrings, if multiple matches are
   * found they will all be modified with the substitution for full details of supported regex
   * syntax see: http://docs.oracle.com/javase/7/docs/technotes/guides/security/SunProviders.html
   *
   * <p>The substitution can specify matching groups using $ followed by the group number for
   * example $1
   *
   * <p>If a null or empty substitution string is provided the regex pattern will be used to remove
   * any substring matching the regex
   *
   * <p>For example:
   *
   * <pre>
  ...
  public HttpRequestModifier withQueryStringParameters(
      QueryParametersModifier queryStringParameters) {
  ...
  public HttpRequestModifier withQueryStringParameters(
      Parameters add, Parameters replace, List<String> remove) {
    this.queryStringParameters =
        new QueryParametersModifier().withAdd(add).withReplace(replace).withRemove(remove);
    this.hashCode = 0;
    return this;
  }

  public HttpRequestModifier withQueryStringParameters(
      List<Parameter> add, List<Parameter> replace, List<String> remove) {
    this.queryStringParameters =
        new QueryParametersModifier()
  ...
  public HttpRequestModifier withHeaders(
      List<Header> add, List<Header> replace, List<String> remove) {
    this.headers =
        new HeadersModifier()
  ...
  public HttpRequestModifier withCookies(
      List<Cookie> add, List<Cookie> replace, List<String> remove) {
    this.cookies =
        new CookiesModifier()
