...
public class EchoWebSocketServerHandler extends ChannelInboundHandlerAdapter {
  ...
  private static final String UPGRADE_CHANNEL_FOR_CALLBACK_WEB_SOCKET_URI =
  ...
  public void channelRead(ChannelHandlerContext ctx, Object msg) {
    ...
    try {
      if (msg instanceof FullHttpRequest
          && ((FullHttpRequest) msg).uri().equals(UPGRADE_CHANNEL_FOR_CALLBACK_WEB_SOCKET_URI)) {
  ...
  private void upgradeChannel(final ChannelHandlerContext ctx, FullHttpRequest httpRequest) {
    ...
    if (handshaker == null) {
      ...
      final String clientId =
          httpRequest.headers().contains(CLIENT_REGISTRATION_ID_HEADER)
                                  ...
                                  if (MockServerLogger.isEnabled(Level.TRACE)) {
                                    mockServerLogger.logEvent(
                                        new LogEntry()
                                            .setLogLevel(Level.TRACE)
                                            .setMessageFormat(
                                                "unregistering callback for client " + clientId));
                                  }
  ...
  private void handleWebSocketFrame(final ChannelHandlerContext ctx, WebSocketFrame frame) {
    if (frame instanceof CloseWebSocketFrame) {
      ...
      throw new UnsupportedOperationException(
          frame.getClass().getName() + " frame types not supported");
    }
  }
