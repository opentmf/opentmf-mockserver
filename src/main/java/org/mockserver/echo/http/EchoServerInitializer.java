...
public class EchoServerInitializer extends ChannelInitializer<SocketChannel> {
  ...
  EchoServerInitializer(
      ...
      List<Channel> websocketChannels,
      List<TextWebSocketFrame> textWebSocketFrames) {
    ...
    if (!secure && error == EchoServer.Error.CLOSE_CONNECTION) {
      throw new IllegalArgumentException(
          "Error type CLOSE_CONNECTION is not supported in non-secure mode");
    }
    this.error = error;
  ...
  public void initChannel(SocketChannel channel) {
    ...
    if (secure) {
      ...
      pipeline.addLast(
          new EchoServerHttpOrHttp2Initializer(
              mockServerLogger,
              channel,
              this::configureHttp1Pipeline,
              this::configureHttp2Pipeline));
    } else {
  ...
  private void configureHttp1Pipeline(SocketChannel channel, ChannelPipeline pipeline) {
    ...
    pipeline.addLast(
        new EchoWebSocketServerHandler(
            mockServerLogger, registeredClients, websocketChannels, textWebSocketFrames, secure));
    pipeline.addLast(
        new MockServerHttpServerCodec(
            configuration, mockServerLogger, secure, null, channel.localAddress().getPort()));
    pipeline.addLast(
  ...
  private void configureHttp2Pipeline(SocketChannel channel, ChannelPipeline pipeline) {
    ...
    if (isEnabled(TRACE)) {
      http2ConnectionHandlerBuilder.frameLogger(
          new Http2FrameLogger(LogLevel.TRACE, EchoServerInitializer.class.getName()));
    }
