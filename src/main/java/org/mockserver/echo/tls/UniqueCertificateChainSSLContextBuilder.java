...
public class UniqueCertificateChainSSLContextBuilder {
  ...
  public static SSLContext uniqueCertificateChainSSLContext(Configuration configuration)
      throws Exception {
    // ssl context
    SSLContext sslContext = SSLContext.getInstance("TLSv1.2");
    sslContext.init(
        new KeyManager[] {new UniqueCertificateChainX509KeyManager(configuration)},
  ...
  private static class UniqueCertificateChainX509KeyManager implements X509KeyManager {
    ...
    private UniqueCertificateChainX509KeyManager(Configuration configuration) {
      ...
      boolean originalDynamicallyCreateCertificateAuthorityCertificate =
          configuration.dynamicallyCreateCertificateAuthorityCertificate();
      String originalDirectoryToSaveDynamicSSLCertificate =
      ...
      try {
        File tempDirectory =
            new File(
                File.createTempFile("prefix", "suffix").getParentFile().getAbsolutePath()
                    + "/"
                    + TSID.Factory.getTsid());
        if (!tempDirectory.mkdir()) {
          throw new RuntimeException(
              "Exception creating temporary directory for test certificates " + tempDirectory);
        }
        ...
        KeyAndCertificateFactory keyAndCertificateFactory =
            KeyAndCertificateFactoryFactory.createKeyAndCertificateFactory(
        ...
        configuration.dynamicallyCreateCertificateAuthorityCertificate(
            originalDynamicallyCreateCertificateAuthorityCertificate);
        configuration.directoryToSaveDynamicSSLCertificate(
            originalDirectoryToSaveDynamicSSLCertificate);
