...
public class JWKGenerator {
  ...
  public String generateJWK(AsymmetricKeyPair asymmetricKeyPair) {
    try {
      ...
      return objectWriter.writeValueAsString(
          ImmutableMap.of("keys", Collections.singletonList(singleKey)));
  ...
  private Map<String, Object> getRSAJWK(AsymmetricKeyPair asymmetricKeyPair) {
    return new RSAKey.Builder(
            AsymmetricKeyConverter.getRSAPublicKey(
                asymmetricKeyPair.getKeyPair().getPublic().getEncoded()))
  ...
  private Map<String, Object> getEllipticCurveJWK(
      AsymmetricKeyPair asymmetricKeyPair, Curve curve) {
    return new ECKey.Builder(
            curve,
            AsymmetricKeyConverter.getECPublicKey(
                asymmetricKeyPair.getKeyPair().getPublic().getEncoded()))
        .keyID(asymmetricKeyPair.getKeyId())
        .keyUse(KeyUse.SIGNATURE)
        .algorithm(new Algorithm(asymmetricKeyPair.getAlgorithm().getJwtAlgorithm()))
