...
public class ImmutableEntry extends Pair<NottableString, NottableString>
  ...
  public static ImmutableEntry entry(
      RegexStringMatcher regexStringMatcher, NottableString key, NottableString value) {
    return new ImmutableEntry(regexStringMatcher, key, value);
  }
      ...
      if (matcherItem != null) {
        for (int j = 0; j < matched.size(); j++) {
          ...
          if (matchedItem != null) {
            ...
            } else if (matcherItem.getKey().isOptional()
                && !contains(regexStringMatcher, matchedKeys, matcherItem.getKey())) {
              matchingMatchedIndexes.add(j);
              matchingMatcherIndexes.add(i);
            } else if (matchedItem.getKey().isOptional()
                && !contains(regexStringMatcher, matcherKeys, matchedItem.getKey())) {
              matchingMatchedIndexes.add(j);
              matchingMatcherIndexes.add(i);
            }
          }
  ...
  private static boolean contains(
      RegexStringMatcher regexStringMatcher,
      Set<NottableString> matchedKeys,
      NottableString matcherItem) {
