...
public class NettySslContextFactory {
  ...
  public static UnaryOperator<SslContextBuilder> sslServerContextBuilderCustomizer =
      UnaryOperator.identity();
  public static UnaryOperator<SslContextBuilder> sslClientContextBuilderCustomizer =
      UnaryOperator.identity();
  ...
  public NettySslContextFactory(
    ...
    keyAndCertificateFactory =
        createKeyAndCertificateFactory(configuration, mockServerLogger, forServer);
    System.setProperty("https.protocols", configuration.tlsProtocols());
    nettySslContextFactoryCustomizer.accept(this);
    if (configuration.proactivelyInitialiseTLS()) {
      createServerSslContext();
  ...
  public NettySslContextFactory withClientSslContextBuilderFunction(
      ...
      try {
        ...
        if (forwardProxyClient) {
          switch (configuration.forwardProxyTLSX509CertificatesTrustManagerType()) {
            ...
            case JVM:
              ...
              sslContextBuilder.trustManager(
                  jvmCAX509TrustCertificates(mockServerX509Certificates));
              break;
            case CUSTOM:
          ...
          if (isNotBlank(configuration.tlsMutualAuthenticationCertificateChain())) {
            mockServerX509Certificates.addAll(
                x509ChainFromPEMFile(configuration.tlsMutualAuthenticationCertificateChain()));
            mockServerX509Certificates.add(
                keyAndCertificateFactory.certificateAuthorityX509Certificate());
          } else {
            mockServerX509Certificates.add(
        ...
        clientSslContext =
            instanceClientSslContextBuilderFunction.apply(
  ...
  private X509Certificate[] forwardProxyCertificateChain() {
    if (isNotBlank(configuration.forwardProxyCertificateChain())) {
      return x509ChainFromPEMFile(configuration.forwardProxyCertificateChain())
  ...
  private X509Certificate[] customCAX509TrustCertificates() {
    ...
    x509Certificates.addAll(
        x509ChainFromPEMFile(configuration.forwardProxyTLSCustomTrustX509Certificates()));
  ...
  public synchronized SslContext createServerSslContext() {
    if (serverSslContext == null
      ...
      try {
        ...
        serverSslContext = sslServerContextBuilderCustomizer.apply(sslContextBuilder).build();
        ...
        mockServerLogger.logEvent(
            new LogEntry()
                ...
                .setMessageFormat(
                    "exception creating SSL context for server" + throwable.getMessage())
  ...
  public X509Certificate[] trustCertificateChain(String tlsMutualAuthenticationCertificateChain) {
    if (isNotBlank(tlsMutualAuthenticationCertificateChain)) {
      List<X509Certificate> x509Certificates =
          x509ChainFromPEMFile(tlsMutualAuthenticationCertificateChain);
      x509Certificates.add(keyAndCertificateFactory.certificateAuthorityX509Certificate());
      return x509Certificates.toArray(new X509Certificate[0]);
    } else {
