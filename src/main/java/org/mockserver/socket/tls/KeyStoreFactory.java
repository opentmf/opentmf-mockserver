...
public class KeyStoreFactory {
  ...
  public final String keyStoreFileName =
      "mockserver_keystore_" + UUIDService.getUUID() + "_" + KEY_STORE_TYPE;

  /**
   * Enforce TLS 1.2 if available, since it's not default up to Java 8.
   *
   * <p>Java 7 disables TLS 1.1 and 1.2 for clients. From <a
   * href="http://docs.oracle.com/javase/7/docs/technotes/guides/security/SunProviders.html">Java
   * Cryptography Architecture Oracle Providers Documentation:</a> Although SunJSSE in the Java SE 7
   * release supports TLS 1.1 and TLS 1.2, neither version is enabled by default for client
   * connections. Some servers do not implement forward compatibility correctly and refuse to talk
   * to TLS 1.1 or TLS 1.2 clients. For interoperability, SunJSSE does not enable TLS 1.1 or TLS 1.2
   * by default for client connections.
   */
  private static final String SSL_CONTEXT_PROTOCOL = "TLSv1.2";

  /**
   * {@link SSLContext}: Every implementation of the Java platform is required to support the
   * following standard SSLContext protocol: TLSv1
   */
  ...
  public KeyStore loadOrCreateKeyStore(
    ...
    return savePrivateKeyAndX509InKeyStore(
        ...
        privateKey,
        KEY_STORE_PASSWORD.toCharArray(),
        new X509Certificate[] {x509Certificate, certificateAuthorityX509Certificate},
        trustX509CertificateChain);
  }
  ...
  private KeyStore savePrivateKeyAndX509InKeyStore(
    ...
    try {
      ...
      keyStore.setKeyEntry(
          KeyStoreFactory.KEY_STORE_CERT_ALIAS, privateKey, keyStorePassword, chain);
