...
public abstract class KeysToMultiValuesDeserializer<T extends KeysToMultiValues<?, ?>> extends StdDeserializer<T> {
    ...
    private T deserializeObject(JsonParser jsonParser, DeserializationContext ctxt) {
        T entries = build();
    ...
    private T deserializeArray(JsonParser jsonParser, DeserializationContext ctxt) {
        T entries = build();
        NottableString key = string("");
        NottableString[] values = null;
        while (true) {
