...
public class HttpRequestToJavaSerializer implements ToJavaSerializer<HttpRequest> {
  ...
  public String serialize(int numberOfSpacesToIndent, HttpRequest request) {
    ...
    if (request != null) {
      ...
      outputQueryStringParameter(
          numberOfSpacesToIndent + 1, output, request.getQueryStringParameterList());
      ...
      if (request.getSocketAddress() != null) {
        ...
        output.append(
            new SocketAddressToJavaSerializer()
                .serialize(numberOfSpacesToIndent + 2, request.getSocketAddress()));
        appendNewLineAndIndent((numberOfSpacesToIndent + 1) * INDENT_SIZE, output);
        output.append(")");
      }
      if (request.getBody() != null) {
        if (request.getBody() instanceof JsonBody) {
          ...
          output
              .append("new JsonBody(\"")
              .append(StringEscapeUtils.escapeJava(jsonBody.getValue()))
              .append("\", JsonBodyMatchType.")
              .append(jsonBody.getMatchType())
              .append(")");
          output.append(")");
          ...
          output
              .append("new JsonPathBody(\"")
              .append(StringEscapeUtils.escapeJava(jsonPathBody.getValue()))
              .append("\")");
          output.append(")");
          ...
          output
              .append("new JsonSchemaBody(\"")
              .append(StringEscapeUtils.escapeJava(jsonSchemaBody.getValue()))
              .append("\")");
          output.append(")");
          ...
          List<Parameter> bodyParameters =
              ((ParameterBody) request.getBody()).getValue().getEntries();
          output.append(
              new ParameterToJavaSerializer()
                  .serializeAsJava(numberOfSpacesToIndent + 3, bodyParameters));
          appendNewLineAndIndent((numberOfSpacesToIndent + 2) * INDENT_SIZE, output);
          output.append(")");
          appendNewLineAndIndent((numberOfSpacesToIndent + 1) * INDENT_SIZE, output);
          output.append(")");
          ...
          output
              .append(".withBody(new Base64Converter().base64StringToBytes(\"")
  ...
  private void outputQueryStringParameter(
      int numberOfSpacesToIndent, StringBuffer output, List<Parameter> parameters) {
  ...
  private void outputCookies(
      int numberOfSpacesToIndent, StringBuffer output, List<Cookie> cookies) {
    if (cookies.size() > 0) {
  ...
  private void outputHeaders(
      int numberOfSpacesToIndent, StringBuffer output, List<Header> headers) {
    if (headers.size() > 0) {
  ...
  private <T extends ObjectWithReflectiveEqualsHashCodeToString> void appendObject(
