...
public class JsonBodyDTOSerializer extends StdSerializer<JsonBodyDTO> {
  ...
  public void serialize(
      JsonBodyDTO jsonBodyDTO, JsonGenerator jgen, SerializationContext provider) {
    ...
    boolean contentTypeNonDefault =
        jsonBodyDTO.getContentType() != null
            && !jsonBodyDTO.getContentType().equals(JsonBody.DEFAULT_JSON_CONTENT_TYPE.toString());
    boolean matchTypeNonDefault = jsonBodyDTO.getMatchType() != JsonBody.DEFAULT_MATCH_TYPE;
    if (serialiseDefaultValues
        || notNonDefault
        || optionalNonDefault
        || contentTypeNonDefault
        || matchTypeNonDefault) {
