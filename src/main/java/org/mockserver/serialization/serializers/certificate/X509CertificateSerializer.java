...
public class X509CertificateSerializer extends StdSerializer<X509Certificate> {
  ...
  public void serialize(
      X509Certificate x509Certificate, JsonGenerator jgen, SerializationContext provider) {
    ...
    if (isNotBlank(x509Certificate.getIssuerDistinguishedName())) {
      jgen.writePOJOProperty(
          "issuerDistinguishedName", x509Certificate.getIssuerDistinguishedName());
    }
    if (isNotBlank(x509Certificate.getSubjectDistinguishedName())) {
      jgen.writePOJOProperty(
          "subjectDistinguishedName", x509Certificate.getSubjectDistinguishedName());
    }
