  ...
  public String serialize(ExpectationId expectationId) {
    try {
      ...
      throw new RuntimeException(
          "Exception while serializing ExpectationId to JSON with value " + expectationId, e);
    }
  }
    ...
    try {
      ...
      mockServerLogger.logEvent(
          new LogEntry()
              .setLogLevel(Level.ERROR)
              .setMessageFormat(
                  "exception while serializing ExpectationId to JSON with value "
                      + Arrays.asList(expectationIds))
              .setThrowable(e));
      throw new RuntimeException(
          "Exception while serializing ExpectationId to JSON with value "
              + Arrays.asList(expectationIds),
        ...
        try {
          ...
          mockServerLogger.logEvent(
              new LogEntry()
                  .setLogLevel(Level.ERROR)
                  .setMessageFormat(
                      "exception while parsing{}for ExpectationId " + throwable.getMessage())
                  .setArguments(jsonExpectationId)
                  .setThrowable(throwable));
          throw new IllegalArgumentException(
              "exception while parsing [" + jsonExpectationId + "] for ExpectationId", throwable);
        }
