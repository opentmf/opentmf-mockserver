...
public class OpenAPIExpectationSerializer implements Serializer<OpenAPIExpectation> {
  ...
  public String serialize(OpenAPIExpectation expectation) {
    if (expectation != null) {
      try {
        return objectWriter.writeValueAsString(new OpenAPIExpectationDTO(expectation));
      } catch (Exception e) {
  ...
  public OpenAPIExpectation deserialize(String jsonOpenAPIExpectation) {
    if (isBlank(jsonOpenAPIExpectation)) {
      ...
      if (validationErrors.isEmpty()) {
        ...
        try {
          OpenAPIExpectationDTO expectationDTO =
              objectMapper.readValue(jsonOpenAPIExpectation, OpenAPIExpectationDTO.class);
          ...
          mockServerLogger.logEvent(
              new LogEntry()
                  .setLogLevel(Level.ERROR)
                  .setMessageFormat(
                      "exception while parsing{}for OpenAPIExpectation " + throwable.getMessage())
                  .setArguments(jsonOpenAPIExpectation)
                  .setThrowable(throwable));
          throw new IllegalArgumentException(
              "exception while parsing [" + jsonOpenAPIExpectation + "] for OpenAPIExpectation",
              throwable);
        }
        return expectation;
      } else {
        throw new IllegalArgumentException(
            StringUtils.removeEndIgnoreCase(
                formatLogMessage(
                    "incorrect openapi expectation json format for:{}schema validation errors:{}",
                    jsonOpenAPIExpectation,
                    validationErrors),
                "\n"));
      }
  ...
  public List<Expectation> deserializeToExpectations(String jsonOpenAPIExpectation) {
    throw new UnsupportedOperationException("OpenAPI support is not included in this build");
  }
    ...
    if (isBlank(jsonOpenAPIExpectations)) {
      throw new IllegalArgumentException(
          "1 error:"
              ...
              + jsonOpenAPIExpectations
              + "\"");
    } else {
      List<String> jsonOpenAPIExpectationList =
          jsonArraySerializer.splitJSONArray(jsonOpenAPIExpectations);
