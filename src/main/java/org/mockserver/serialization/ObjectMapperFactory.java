...
public class ObjectMapperFactory {
    ...
    public static ObjectMapper createObjectMapper(ValueSerializer... additionValueSerializers) {
        if (additionValueSerializers == null || additionValueSerializers.length == 0) {
            ...
            return buildObjectMapperWithDeserializerAndSerializers(Collections.emptyList(), Arrays.asList(additionValueSerializers), false);
        }
    }

    public static ObjectMapper createObjectMapper(ValueDeserializer... replacementValueDeserializers) {
        if (replacementValueDeserializers == null || replacementValueDeserializers.length == 0) {
            ...
            return buildObjectMapperWithDeserializerAndSerializers(Arrays.asList(replacementValueDeserializers), Collections.emptyList(), false);
        }
    }

    public static ObjectWriter createObjectMapper(boolean pretty, boolean serialiseDefaultValues, ValueSerializer... additionValueSerializers) {
        if (additionValueSerializers == null || additionValueSerializers.length == 0) {
            ...
            if (pretty) {
                return buildObjectMapperWithDeserializerAndSerializers(Collections.emptyList(), Arrays.asList(additionValueSerializers), serialiseDefaultValues).writerWithDefaultPrettyPrinter();
            } else {
                return buildObjectMapperWithDeserializerAndSerializers(Collections.emptyList(), Arrays.asList(additionValueSerializers), serialiseDefaultValues).writer();
            }
        }
    }
    ...
    private static ObjectMapper buildObjectMapperWithDeserializerAndSerializers(List<ValueDeserializer> replacementValueDeserializers, List<ValueSerializer> replacementValueSerializers, boolean serialiseDefaultValues) {
        ...
        addSerializers(module, replacementValueSerializers.toArray(new ValueSerializer[0]), serialiseDefaultValues);
        return base.rebuild()
            .addModule(module)
            .build();
    }

    private static void addDeserializers(SimpleModule module, ValueDeserializer[] replacementValueDeserializers) {
        ...
        for (ValueDeserializer additionValueDeserializer : replacementValueDeserializers) {
            jsonDeserializersByType.put(additionValueDeserializer.handledType(), additionValueDeserializer);
        }
        for (Map.Entry<Class, ValueDeserializer> additionValueDeserializer : jsonDeserializersByType.entrySet()) {
            module.addDeserializer(additionValueDeserializer.getKey(), additionValueDeserializer.getValue());
        }
    }

    private static void addSerializers(SimpleModule module, ValueSerializer[] replacementValueSerializers, boolean serialiseDefaultValues) {
        ...
        for (Map.Entry<Class, ValueSerializer> additionValueSerializer : jsonSerializersByType.entrySet()) {
