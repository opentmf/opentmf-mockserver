...
public class ExpectationSerializer implements Serializer<Expectation> {
      ...
      if (!printedECMA262Warning) {
        ...
        if (!System.getProperty("java.version").contains("1.8")
          ...
          try {
            this.getClass()
                .getClassLoader()
                .loadClass("jdk.nashorn.api.scripting.NashornScriptEngineFactory");
            System.err.println(
    ...
    if (expectation != null) {
      try {
        ...
        throw new RuntimeException(
            "Exception while serializing expectation to JSON with value " + expectation, e);
      }
    } else {
      return "";
    }
    ...
    try {
      ...
      mockServerLogger.logEvent(
          new LogEntry()
              .setLogLevel(Level.ERROR)
              .setMessageFormat(
                  "exception while serializing expectation to JSON with value "
  ...
  public Expectation deserialize(String jsonExpectation) {
    if (isBlank(jsonExpectation)) {
      ...
      if (validationErrors.isEmpty()) {
        ...
        try {
          ExpectationDTO expectationDTO =
              objectMapper.readValue(jsonExpectation, ExpectationDTO.class);
          ...
          mockServerLogger.logEvent(
              new LogEntry()
                  .setLogLevel(Level.ERROR)
                  .setMessageFormat(
                      "exception while parsing{}for Expectation " + throwable.getMessage())
                  .setArguments(jsonExpectation)
                  .setThrowable(throwable));
          throw new IllegalArgumentException(
              "exception while parsing [" + jsonExpectation + "] for Expectation", throwable);
        }
        return expectation;
      } else {
        throw new IllegalArgumentException(
            StringUtils.removeEndIgnoreCase(
                formatLogMessage(
                    "incorrect expectation json format for:{}schema validation errors:{}",
                    jsonExpectation,
                    validationErrors),
                "\n"));
      }
  ...
  public Expectation[] deserializeArray(
    ...
    if (isBlank(jsonExpectations)) {
      ...
      List<JsonNode> jsonExpectationList =
          jsonArraySerializer.splitJSONArrayToJSONNodes(jsonExpectations);
      if (!jsonExpectationList.isEmpty()) {
        for (int i = 0; i < jsonExpectationList.size(); i++) {
          String jsonExpectation =
              objectMapper
                  .writerWithDefaultPrettyPrinter()
                  .writeValueAsString(jsonExpectationList.get(i));
          ...
          if (jsonExpectationList.get(i).has("specUrlOrPayload")) {
            try {
              expectations.addAll(
                  expectationModifier.apply(
                      jsonExpectation,
                      openAPIExpectationSerializer.deserializeToExpectations(jsonExpectation)));
            ...
            try {
              expectations.addAll(
                  expectationModifier.apply(
                      jsonExpectation, Collections.singletonList(deserialize(jsonExpectation))));
            } catch (IllegalArgumentException iae) {
              validationErrorsList.add(iae.getMessage());
            }
          }
        ...
        if (!validationErrorsList.isEmpty()) {
          if (validationErrorsList.size() > 1) {
            throw new IllegalArgumentException(
                ("[" + NEW_LINE + Joiner.on("," + NEW_LINE + NEW_LINE).join(validationErrorsList))
