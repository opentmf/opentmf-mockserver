        ...
        if (MockServerLogger.isEnabled(DEBUG) && mockServerLogger != null) {
          mockServerLogger.logEvent(
              new LogEntry()
                  .setLogLevel(DEBUG)
                  .setMessageFormat(
                      "error while creating xpath expression for ["
                          + matcher
                          + "] assuming matcher not xpath - "
  ...
  public boolean matches(final MatchDifference context, final String matched) {
    ...
    if (jsonPath == null) {
      if (context != null) {
        context.addDifference(
            mockServerLogger,
            "json path match failed expected:{}found:{}failed because:{}",
            "null",
            matched,
            "json path matcher was null");
      ...
      try {
        ...
        result =
            jsonPathResult instanceof List
    ...
    if (!result && !alreadyLoggedMatchFailure && context != null) {
      context.addDifference(
          ...
          matcher,
          matched,
          "json path did not evaluate to truthy");
    }
