...
public class JsonStringMatcher extends BodyMatcher<String> {
        ...
        if (!result) {
          if (context != null) {
            if (differences.isEmpty()) {
              context.addDifference(
                  mockServerLogger, "json match failed expected:{}found:{}", this.matcher, matched);
            } else {
              context.addDifference(
                  mockServerLogger,
                  ...
                  this.matcher,
                  matched,
                  Joiner.on("," + NEW_LINE).join(differences));
            }
          }
        }
      }
  ...
  private boolean compareNodes(
      JsonNode expected, JsonNode actual, String path, List<String> differences) {
  ...
  private boolean compareObjects(
      ObjectNode expected, ObjectNode actual, String path, List<String> differences) {
    boolean match = true;
  ...
  private boolean compareArrays(
      ArrayNode expected, ArrayNode actual, String path, List<String> differences) {
    if (matchType == MatchType.STRICT) {
  ...
  private boolean compareArraysStrict(
      ArrayNode expected, ArrayNode actual, String path, List<String> differences) {
    boolean match = true;
  ...
  private boolean compareArraysLenient(
      ArrayNode expected, ArrayNode actual, String path, List<String> differences) {
    boolean match = true;
