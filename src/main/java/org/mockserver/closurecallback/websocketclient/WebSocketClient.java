  ...
  private Future<String> register(
    ...
    try {
              ...
              new ChannelInitializer<SocketChannel>() {
                ...
                protected void initChannel(SocketChannel ch) throws URISyntaxException {
                  ...
                  if (MockServerLogger.isEnabled(TRACE)) {
                    ch.pipeline()
                        .addLast(new LoggingHandler(WebSocketClient.class.getName() + "-last"));
                  }
                }
              })
      ...
      registrationFuture.completeExceptionally(
          new WebSocketException("Exception while starting web socket client", e));
    }
    ...
    try {
      ...
      if (deserializedMessage instanceof HttpRequest) {
        ...
        String webSocketCorrelationId =
            request.getFirstHeader(WEB_SOCKET_CORRELATION_ID_HEADER_NAME);
        if (MockServerLogger.isEnabled(TRACE) && mockServerLogger != null) {
          mockServerLogger.logEvent(
              new LogEntry()
                  ...
                  .setMessageFormat(
                      "received request{}over websocket for client "
                          + clientId
                          + " for correlationId "
        ...
        if (expectationCallback != null) {
          try {
            ...
            if (MockServerLogger.isEnabled(TRACE) && mockServerLogger != null) {
              mockServerLogger.logEvent(
                  new LogEntry()
                      ...
                      .setHttpRequest(request)
                      .setMessageFormat(
                          "returning{}for request{}over websocket for client "
                              + clientId
                              + " for correlationId "
                              + webSocketCorrelationId)
            ...
            channel.writeAndFlush(
                new TextWebSocketFrame(webSocketMessageSerializer.serialize(result)));
            ...
            mockServerLogger.logEvent(
                new LogEntry()
                    ...
                    .setHttpRequest(request)
                    .setMessageFormat(
                        "exception thrown while handling callback for request - "
        ...
        HttpRequestAndHttpResponse httpRequestAndHttpResponse =
            (HttpRequestAndHttpResponse) deserializedMessage;
        ...
        if (MockServerLogger.isEnabled(TRACE) && mockServerLogger != null) {
          mockServerLogger.logEvent(
              new LogEntry()
                  ...
                  .setMessageFormat(
                      "received request and response{}over websocket for client "
                          + clientId
                          + " for correlationId "
                          + webSocketCorrelationId)
        ...
        if (expectationForwardResponseCallback != null) {
          try {
            HttpResponse response =
                expectationForwardResponseCallback.handle(httpRequest, httpResponse);
            if (MockServerLogger.isEnabled(TRACE) && mockServerLogger != null) {
              mockServerLogger.logEvent(
                  new LogEntry()
                      .setLogLevel(TRACE)
                      .setHttpRequest(httpRequestAndHttpResponse.getHttpRequest())
                      .setMessageFormat(
                          "returning response{}for request and response{}over websocket for client "
                              + clientId
                              + " for correlationId "
                              + webSocketCorrelationId)
            ...
            channel.writeAndFlush(
                new TextWebSocketFrame(webSocketMessageSerializer.serialize(response)));
          } catch (Throwable throwable) {
            mockServerLogger.logEvent(
                new LogEntry()
                    .setLogLevel(Level.ERROR)
                    ...
                    .setMessageFormat(
                        "exception thrown while handling callback for request and response - "
                            + throwable.getMessage())
                    .setThrowable(throwable));
            channel.writeAndFlush(
                new TextWebSocketFrame(
        ...
        if (MockServerLogger.isEnabled(WARN) && mockServerLogger != null) {
          mockServerLogger.logEvent(
              new LogEntry()
                  .setLogLevel(WARN)
                  .setMessageFormat(
                      "web socket client received a message that isn't HttpRequest or HttpRequestAndHttpResponse{} which has been deserialized as{}")
