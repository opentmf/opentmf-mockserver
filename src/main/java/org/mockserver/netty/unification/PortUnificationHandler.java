...
// Removed: dashboard, proxy, socks imports
...
public class PortUnificationHandler extends ReplayingDecoder<Void> {
  ...
  private static final String PROXIED = "PROXIED_";
  private static final String PROXIED_SECURE = PROXIED + "SECURE_";
  private static final String PROXIED_RESPONSE = "PROXIED_RESPONSE_";
  ...
  private final PreserveHeadersNettyRemoves preserveHeadersNettyRemoves =
  ...
  protected void decode(ChannelHandlerContext ctx, ByteBuf msg, List<Object> out) {
    ...
    if (isTls(msg)) {
      ...
      // SOCKS proxy support removed from this build
      logStage(ctx, "adding HTTP decoders");
      switchToHttp(ctx, msg);
    } else {
      logStage(ctx, "unknown protocol - closing connection");
      ctx.close();
      return;
    }
    ...
    if (!isHttp2Enabled(ctx.channel())) {
      ...
      if (isEnabled(TRACE)) {
        http2ConnectionHandlerBuilder.frameLogger(
            new Http2FrameLogger(LogLevel.TRACE, PortUnificationHandler.class.getName()));
      }
      addLastIfNotPresent(pipeline, http2ConnectionHandlerBuilder.connection(connection).build());
      // TODO(jamesdbloom) consider Http2MultiplexHandler and test behaviour when multiple requests
