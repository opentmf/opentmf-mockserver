...
public class RequestMatchers extends MockServerMatcherNotifier {
    ...
    if (MockServerLogger.isEnabled(TRACE) && mockServerLogger != null) {
      mockServerLogger.logEvent(
          new LogEntry()
              .setLogLevel(TRACE)
              .setMessageFormat(
                  "expectation circular priority queue created, with size "
                      + configuration.maxExpectations()));
    }
  }
              ...
              .map(
                  httpRequestMatcher -> {
                    if (httpRequestMatcher.getExpectation() != null
                        && httpRequestMatcher.getExpectation().getAction() != null) {
  ...
  public void update(Expectation[] expectations, Cause cause) {
    ...
    if (expectations != null) {
      ...
      Set<String> existingKeysForCause =
          httpRequestMatchersByKey.entrySet().stream()
          ...
          .forEach(
              expectation -> {
                ...
                if (!addedIds.contains(expectation.getId())) {
                  ...
                  expectationRequestDefinitions.put(
                      expectation.getId(), expectation.getHttpRequest());
