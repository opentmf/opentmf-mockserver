  ...
  private void handleLocally(
    ...
    try {
      ...
      actionHandler.writeResponseActionResponse(
          callbackResponse, responseWriter, request, httpObjectCallback, synchronous);
            ...
            new LogEntry()
                ...
                .setMessageFormat(
                    "returning{}because client "
                        + clientId
                        + " response callback throw an exception")
                .setArguments(notFoundResponse())
                .setThrowable(throwable));
      }
      actionHandler.writeResponseActionResponse(
          notFoundResponse(), responseWriter, request, httpObjectCallback, synchronous);
    }
  }
          ...
          if (MockServerLogger.isEnabled(TRACE) && mockServerLogger != null) {
            mockServerLogger.logEvent(
                new LogEntry()
                    ...
                    .setHttpRequest(request)
                    .setMessageFormat(
                        "received response over websocket{}for request{}from client "
                            + clientId
                            + " for correlationId "
                            + webSocketCorrelationId)
