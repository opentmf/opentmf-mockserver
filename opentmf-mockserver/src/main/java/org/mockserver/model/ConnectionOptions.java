...
public class ConnectionOptions extends ObjectWithJsonToString {
   ...
   *
   * @param suppressContentLengthHeader if true no "Content-Length" header will be added to the
   *     response
   */
   ...
   *
   * <p>If null (the default value), zero or negative then response will not be chunked If positive
   * and non-null the response will have "Transfer-Encoding: chunked" header and will be chunked
   * into chunks of the size specified (as long as the body is large enough)
   *
  ...
  /**
   * Override the "Connection" header: if true the "Connection" header is specified with a value of
   * "keep-alive" if false the "Connection" header is specified with a value of "close" if not set
   * the "Connection" header will have a a value of "close" unless the request received is HTTP 1.1
   * and contains a "Connection" header with a value of "keep-alive"
   *
   * @param keepAliveOverride if true "keep-alive" is used if false "close" is used for the
   *     "Connection" header
   */
  ...
  /**
   * Override whether the socket is closed after a response is sent: if true the socket will always
   * be closed, if false the socket will never be closed, if not set the socket will be closed
   * unless the request received is HTTP 1.1 and contains a "Connection" header with a value of
   * "keep-alive"
   *
  ...
  /**
   * Override how long to delay before closing socket, this value is ignored if socket isn't going
   * to be closed
   *
