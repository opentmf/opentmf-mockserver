...
public class OpenAPIDefinition extends RequestDefinition {
   ...
   *
   * <p>// Create from a publicly hosted HTTP location (json or yaml)
   * openAPI("https://raw.githubusercontent.com/OAI/OpenAPI-Specification/master/examples/v3.0/petstore-expanded.yaml")
   *
   * <p>// Create from a file on the local filesystem (json or yaml)
   ...
   *
   * <p>// Create from a classpath resource in the /api package (json or yaml)
   * openAPI("org/mockserver/openapi/openapi_petstore_example.json");
   *
   * <p>// Create from an OpenAPI payload (json or yaml) openAPI("{\"openapi\": \"3.0.0\", \"info\":
   * { ...")
   *
   * <p>
   *
   ...
   *
   * <p>// Create from an OpenAPI payload (json or yaml) withSpecUrlOrPayload("{\"openapi\":
   * \"3.0.0\", \"info\": { ...")
   *
   * <p>
   *
  ...
  public OpenAPIDefinition shallowClone() {
    return not(openAPI(), not).withSpecUrlOrPayload(specUrlOrPayload).withOperationId(operationId);
  }

  public OpenAPIDefinition update(OpenAPIDefinition replaceRequest) {
    if (replaceRequest.getSpecUrlOrPayload() != null
        && isNotBlank(replaceRequest.getSpecUrlOrPayload())) {
