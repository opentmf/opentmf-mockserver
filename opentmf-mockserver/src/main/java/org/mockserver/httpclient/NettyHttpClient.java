    ...
    if (!eventLoopGroup.isShuttingDown()) {
      ...
      if (Protocol.HTTP_2.equals(httpRequest.getProtocol())
        ...
        mockServerLogger.logEvent(
            new LogEntry()
                ...
                .setMessageFormat(
                    "HTTP2 requires ALPN but request is not secure (i.e. TLS) so protocol changed to HTTP1"));
      ...
      final HttpClientInitializer clientInitializer =
          new HttpClientInitializer(
              mockServerLogger, forwardProxyClient, nettySslContextFactory, httpProtocol);
      ...
      throw new IllegalStateException(
          "Request sent after client has been stopped - the event loop has been shutdown so it is not possible to send a request");
    }
  }
  ...
  public CompletableFuture<BinaryMessage> sendRequest(
    ...
    if (!eventLoopGroup.isShuttingDown()) {
      ...
      new Bootstrap()
          ...
          .handler(
              new HttpClientInitializer(
                  mockServerLogger, forwardProxyClient, nettySslContextFactory, null))
          .connect(remoteAddress)
          .addListener(
              (ChannelFutureListener)
                  future -> {
      ...
      if (!ignoreErrors) {
        throw new SocketCommunicationException(
            "Response was not received from MockServer after "
                ...
                + " milliseconds, to wait longer please use \"mockserver.maxSocketTimeout\" system property or ConfigurationProperties.maxSocketTimeout(long milliseconds)",
            e.getCause());
      }
