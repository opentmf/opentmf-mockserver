...
public class BCKeyAndCertificateFactory implements KeyAndCertificateFactory {
  ...
  public BCKeyAndCertificateFactory(
  ...
  public void buildAndSaveCertificateAuthorityPrivateKeyAndX509Certificate() {
    if (dynamicallyUpdateCertificateAuthority() && certificateAuthorityCertificateNotYetCreated()) {
      try {
        AsymmetricKeyPairAlgorithm keyGenerationAndSigningAlgorithm =
        ...
        saveAsPEMFile(
            generateCACert(
                keyGenerationAndSigningAlgorithm, caKeyPair.getPublic(), caKeyPair.getPrivate()),
            certificateAuthorityX509CertificatePath(),
            "Certificate Authority X509 Certificate PEM");
        saveAsPEMFile(
            caKeyPair.getPrivate(),
            certificateAuthorityPrivateKeyPath(),
            "Certificate Authority Private Key PEM");
      } catch (Exception e) {
        mockServerLogger.logEvent(
            new LogEntry()
                .setLogLevel(Level.ERROR)
                .setMessageFormat(
                    "exception while generating certificate authority private key and X509 certificate")
                .setThrowable(e));
      }
    }
  }
  ...
  private String certificateAuthorityPrivateKeyPath() {
    if (dynamicallyUpdateCertificateAuthority()) {
      configuration.certificateAuthorityPrivateKey(
          new File(
                  new File(configuration.directoryToSaveDynamicSSLCertificate()),
      ...
      if (MockServerLogger.isEnabled(TRACE) && mockServerLogger != null) {
        mockServerLogger.logEvent(
            new LogEntry()
                ...
                .setArguments(
                    FilePath.absolutePathFromClassPathOrPath(certificateAuthorityPrivateKeyPath()),
        ...
        mockServerLogger.logEvent(
            new LogEntry()
                ...
                .setArguments(
                    FilePath.absolutePathFromClassPathOrPath(
                        certificateAuthorityPrivateKeyPath())));
      }
    }
  ...
  public X509Certificate certificateAuthorityX509Certificate() {
    if (certificateAuthorityX509Certificate == null) {
      ...
      certificateAuthorityX509Certificate =
          x509FromPEMFile(certificateAuthorityX509CertificatePath());
  ...
  private X509Certificate generateCACert(
      AsymmetricKeyPairAlgorithm keyGenerationAndSigningAlgorithm,
      PublicKey publicKey,
    ...
    X509v3CertificateBuilder builder =
        new JcaX509v3CertificateBuilder(
            issuerName, serial, NOT_BEFORE, NOT_AFTER, issuerName, publicKey);
    builder.addExtension(
        Extension.subjectKeyIdentifier, false, createSubjectKeyIdentifier(publicKey));
    ...
    if (customPrivateKeyAndCertificateProvided()) {
      try {
        ...
        if (configuration.preventCertificateDynamicUpdate()
            || configuration.proactivelyInitialiseTLS()) {
  ...
  private String x509CertificatePath() {
    configuration.x509CertificatePath(
        new File(new File(configuration.directoryToSaveDynamicSSLCertificate()), "Certificate.pem")
            .getAbsolutePath());
  ...
  private X509Certificate generateLeafCert(
      AsymmetricKeyPairAlgorithm keyGenerationAndSigningAlgorithm,
      PublicKey publicKey,
      ...
      String domain,
      Set<String> subjectAlternativeNameDomains,
      Set<String> subjectAlternativeNameIps)
      throws Exception {
    ...
    X509v3CertificateBuilder builder =
        new JcaX509v3CertificateBuilder(issuer, serial, NOT_BEFORE, NOT_AFTER, subject, publicKey);
    builder.addExtension(
        Extension.subjectKeyIdentifier, false, createSubjectKeyIdentifier(publicKey));
    ...
    if (subjectAlternativeNameDomains != null) {
      ...
      for (String subjectAlternativeNameDomain : subjectAlternativeNameDomains) {
        subjectAlternativeNames.add(
            new GeneralName(GeneralName.dNSName, subjectAlternativeNameDomain));
      }
    }
    if (subjectAlternativeNameIps != null) {
      for (String subjectAlternativeNameIp : subjectAlternativeNameIps) {
        if (IPAddress.isValidIPv6WithNetmask(subjectAlternativeNameIp)
          ...
          subjectAlternativeNames.add(
              new GeneralName(GeneralName.iPAddress, subjectAlternativeNameIp));
        }
      }
    }
    if (subjectAlternativeNames.size() > 0) {
      DERSequence subjectAlternativeNamesExtension =
          new DERSequence(subjectAlternativeNames.toArray(new ASN1Encodable[0]));
      builder.addExtension(
          Extension.subjectAlternativeName, false, subjectAlternativeNamesExtension);
    }
    X509Certificate signedX509Certificate =
        signCertificate(keyGenerationAndSigningAlgorithm, builder, certificateAuthorityPrivateKey);
  ...
  private X509Certificate signCertificate(
    ...
    ContentSigner signer =
        new JcaContentSignerBuilder(keyGenerationAndSigningAlgorithm.getSigningAlgorithm())
  ...
  private void saveAsPEMFile(Object object, String absolutePath, String type) throws IOException {
    ...
    try (FileWriter pemfileWriter =
        new FileWriter(FileCreator.createFileIfNotExists(type, new File(absolutePath)))) {
