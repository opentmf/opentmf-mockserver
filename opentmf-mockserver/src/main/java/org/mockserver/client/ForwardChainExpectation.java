...
public class ForwardChainExpectation {
  ...
  ForwardChainExpectation(
      ...
      MockServerEventBus mockServerEventBus,
  ...
  /**
   * Set priority of this expectation which is used to determine the matching order of expectations
   * when a request is received.
   *
   * <p>Matching is ordered by priority (highest first) then creation (earliest first).
   *
  ...
  /**
   * Evaluate Velocity or JavaScript template to generate response to return when expectation is
   * matched
   *
  ...
  /**
   * Call method on local class in same JVM implementing ExpectationResponseCallback to generate
   * response to return when expectation is matched
   *
   * <p>The callback class must: - implement org.mockserver.mock.action.ExpectationResponseCallback
  ...
  /**
   * Call method on object locally or remotely (over web socket) to generate response to return when
   ...
   * @return added or updated expectations
   */
  public Expectation[] respond(
      final ExpectationResponseCallback expectationResponseCallback, Delay delay) {
    expectation.thenRespond(
        new HttpObjectCallback()
            .withClientId(registerWebSocketClient(expectationResponseCallback, null))
            .withDelay(delay));
  ...
  /**
   * Evaluate Velocity or JavaScript template to generate request to forward when expectation is
   * matched
   *
   * @param httpTemplate Velocity or JavaScript template used to generate response
   * @return added or updated expectations
   ...
   *
   * <p>The callback class must: - implement org.mockserver.mock.action.ExpectationForwardCallback
   * or org.mockserver.mock.action.ExpectationForwardAndResponseCallback - have a zero argument
   * constructor - be available in the classpath of the MockServer
   *
   * @param httpClassCallback class to callback as a fully qualified class name, i.e.
   *     "com.foo.MyExpectationResponseCallback"
   * @return added or updated expectations
  ...
  /**
   * Call method on object locally or remotely (over web socket) to generate request to forward when
   * expectation is matched
   *
        ...
        new HttpObjectCallback()
            ...
            .withClientId(
                registerWebSocketClient(
                    expectationForwardCallback, expectationForwardResponseCallback)));
    return mockServerClient.upsert(expectation);
  }
   ...
   */
  public Expectation[] forward(
      final ExpectationForwardCallback expectationForwardCallback, final Delay delay) {
    expectation.thenForward(
        new HttpObjectCallback()
            .withClientId(registerWebSocketClient(expectationForwardCallback, null))
            .withDelay(delay));
  ...
  /**
   * Override fields, headers, and cookies etc in request being forwarded with specified fields,
   * headers and cookies, etc in the specified request when expectation is matched
   *
  ...
  private <T extends HttpMessage> String registerWebSocketClient(
      ExpectationCallback<T> expectationCallback,
    ...
    try {
      ...
      final WebSocketClient<T> webSocketClient =
          new WebSocketClient<>(
              new NioEventLoopGroup(
                  configuration.webSocketClientEventLoopThreadCount(),
                  new Scheduler.SchedulerThreadFactory(
                      WebSocketClient.class.getSimpleName() + "-eventLoop")),
              clientId,
              mockServerLogger);
      final Future<String> register =
          webSocketClient.registerExpectationCallback(
              expectationCallback,
