          ...
          for (NottableString matcherKey : subset.backingMap.keySet()) {
            ...
            for (NottableString matchedValue : matchedValuesForKey) {
              ...
              for (NottableString matcherValue : matcherValuesForKey) {
                // match first as list
                if (matcherValue instanceof NottableSchemaString
                    && ((NottableSchemaString) matcherValue)
                        .matches(mockServerLogger, context, matchedValuesForKey)) {
                  matchesValue = true;
                  break;
                  // otherwise match item by item
                } else if (regexStringMatcher.matches(
