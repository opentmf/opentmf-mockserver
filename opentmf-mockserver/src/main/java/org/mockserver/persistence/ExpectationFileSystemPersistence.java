...
public class ExpectationFileSystemPersistence implements MockServerMatcherListener {
  ...
  public ExpectationFileSystemPersistence(
    ...
    if (configuration.persistExpectations()) {
      ...
      this.initializationPathMatchesPersistencePath =
          FilePath.expandFilePathGlobs(configuration.initializationJsonPath())
              .contains(configuration.persistedExpectationsPath());
  ...
  public void updated(RequestMatchers requestMatchers, MockServerMatcherNotifier.Cause cause) {
    ...
    if (cause == MockServerMatcherNotifier.Cause.API
        || cause.getType() == MockServerMatcherNotifier.Cause.Type.CLASS_INITIALISER
        || !initializationPathMatchesPersistencePath) {
      fileWriteLock.lock();
      try {
        try {
          ...
          mockServerLogger.logEvent(
              new LogEntry()
                  .setLogLevel(Level.ERROR)
                  .setMessageFormat(
                      "exception while persisting expectations to " + filePath.toString())
                  .setThrowable(throwable));
        }
      } finally {
