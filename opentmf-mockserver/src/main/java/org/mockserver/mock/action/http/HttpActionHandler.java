...
public class HttpActionHandler {
  ...
  private HttpOverrideForwardedRequestActionHandler
      httpOverrideForwardedRequestCallbackActionHandler;
  ...
  public HttpActionHandler(
    ...
    this.httpClient =
        new NettyHttpClient(
            configuration, mockServerLogger, eventLoopGroup, true, nettySslContextFactory);
  }
    ...
    if (expectation != null && expectation.getAction() != null) {
      ...
      switch (action.getType()) {
                    ...
                    handleAnyException(
                        ...
                        () -> {
                          final HttpResponse response =
                              getHttpResponseActionHandler().handle((HttpResponse) action);
                          writeResponseActionResponse(
                              response, responseWriter, request, action, synchronous);
        ...
        case RESPONSE_TEMPLATE:
          {
            throw new UnsupportedOperationException(
                "RESPONSE_TEMPLATE action type is not supported in this build");
          }
            ...
            scheduler.schedule(
                () ->
                    handleAnyException(
                        ...
                        () -> {
                          final HttpResponse response =
                              getHttpResponseClassCallbackActionHandler()
        ...
        case RESPONSE_OBJECT_CALLBACK:
          {
            scheduler.schedule(
                () ->
                    getHttpResponseObjectCallbackActionHandler()
                        .handle(
                    ...
                    handleAnyException(
                        ...
                        () -> {
                          final HttpForwardActionResult responseFuture =
                              getHttpForwardActionHandler().handle((HttpForward) action, request);
                          writeForwardActionResponse(
                              responseFuture, responseWriter, request, action, synchronous);
                          expectationPostProcessor.run();
                        }),
                synchronous,
                action.getDelay());
        ...
        case FORWARD_TEMPLATE:
          {
            throw new UnsupportedOperationException(
                "FORWARD_TEMPLATE action type is not supported in this build");
          }
            ...
            scheduler.schedule(
                () ->
                    handleAnyException(
                        ...
                        () -> {
                          final HttpForwardActionResult responseFuture =
                              getHttpForwardClassCallbackActionHandler()
                                  .handle((HttpClassCallback) action, request);
                          writeForwardActionResponse(
                              responseFuture, responseWriter, request, action, synchronous);
                          expectationPostProcessor.run();
        ...
        case FORWARD_OBJECT_CALLBACK:
          {
            scheduler.schedule(
                () ->
                    getHttpForwardObjectCallbackActionHandler()
                        .handle(
                            HttpActionHandler.this,
                            (HttpObjectCallback) action,
                            request,
            ...
            scheduler.schedule(
                () ->
                    handleAnyException(
                        ...
                        () -> {
                          final HttpForwardActionResult responseFuture =
                              getHttpOverrideForwardedRequestCallbackActionHandler()
                                  .handle((HttpOverrideForwardedRequest) action, request);
                          writeForwardActionResponse(
                              responseFuture, responseWriter, request, action, synchronous);
                          expectationPostProcessor.run();
                        }),
                        ...
                        () -> {
                          ...
                          mockServerLogger.logEvent(
                              new LogEntry()
                                  ...
                                  .setArguments(
                                      action, request, action, action.getExpectationId()));
                          expectationPostProcessor.run();
                        }),
                synchronous,
                action.getDelay());
        ...
        if (MockServerLogger.isEnabled(TRACE) && mockServerLogger != null) {
          mockServerLogger.logEvent(
              new LogEntry()
                  ...
                  .setCorrelationId(request.getLogCorrelationId())
                  .setMessageFormat(
                      "received \"x-forwarded-by\" header caused by exploratory HTTP proxy or proxy loop - falling back to no proxy:{}")
                  .setArguments(request));
        }
                  ...
                  if (potentiallyHttpProxy && connectionException(throwable)) {
                    if (MockServerLogger.isEnabled(TRACE) && mockServerLogger != null) {
                      mockServerLogger.logEvent(
                          new LogEntry()
                              ...
                              .setCorrelationId(request.getLogCorrelationId())
                              .setMessageFormat(
                                  "failed to connect to proxied socket due to exploratory HTTP proxy for:{}due to:{}falling back to no proxy")
                    ...
                    mockServerLogger.logEvent(
                        new LogEntry()
                            ...
                            .setHttpRequest(request)
                            .setMessageFormat(
                                "TLS handshake exception while proxying request{}to remote address{}with channel"
        ...
        () -> {
          mockServerLogger.logEvent(
              new LogEntry()
                  ...
                  .setExpectationId(action.getExpectationId())
                  .setMessageFormat(
                      "returning response:{}for request:{}for action:{}from expectation:{}")
                ...
                new LogEntry()
                    ...
                    .setMessageFormat(
                        "returning response:{}for forwarded request"
                            ...
                            + NEW_LINE
                            + NEW_LINE
                            + " in curl:{}for action:{}from expectation:{}")
                    .setArguments(
                        response,
                        responseFuture.getHttpRequest(),
                        httpRequestToCurlSerializer.toCurl(
                            responseFuture.getHttpRequest(), responseFuture.getRemoteAddress()),
                        action,
    ...
    if (connectionException(exception)) {
      ...
      mockServerLogger.logEvent(
          new LogEntry()
              ...
              .setMessageFormat(
                  exception != null
                      ? isNotBlank(exception.getMessage())
  ...
  private HttpResponseClassCallbackActionHandler getHttpResponseClassCallbackActionHandler() {
    if (httpResponseClassCallbackActionHandler == null) {
      httpResponseClassCallbackActionHandler =
          new HttpResponseClassCallbackActionHandler(mockServerLogger);
    }
  ...
  private HttpResponseObjectCallbackActionHandler getHttpResponseObjectCallbackActionHandler() {
    if (httpResponseObjectCallbackActionHandler == null) {
      httpResponseObjectCallbackActionHandler =
          new HttpResponseObjectCallbackActionHandler(httpStateHandler);
    }
  ...
  private HttpForwardClassCallbackActionHandler getHttpForwardClassCallbackActionHandler() {
    if (httpForwardClassCallbackActionHandler == null) {
      httpForwardClassCallbackActionHandler =
          new HttpForwardClassCallbackActionHandler(mockServerLogger, httpClient);
    }
  ...
  private HttpForwardObjectCallbackActionHandler getHttpForwardObjectCallbackActionHandler() {
    if (httpForwardObjectCallbackActionHandler == null) {
      httpForwardObjectCallbackActionHandler =
          new HttpForwardObjectCallbackActionHandler(httpStateHandler, httpClient);
    }
  ...
  private HttpOverrideForwardedRequestActionHandler
    ...
    if (httpOverrideForwardedRequestCallbackActionHandler == null) {
      httpOverrideForwardedRequestCallbackActionHandler =
          new HttpOverrideForwardedRequestActionHandler(mockServerLogger, httpClient);
    }
  ...
  public static void setRemoteAddress(
      final ChannelHandlerContext ctx, final InetSocketAddress inetSocketAddress) {
