...
public class WebSocketClientHandler extends SimpleChannelInboundHandler<Object> {
  ...
  public void channelInactive(ChannelHandlerContext ctx) {
    if (MockServerLogger.isEnabled(TRACE) && mockServerLogger != null) {
      mockServerLogger.logEvent(
          new LogEntry().setLogLevel(TRACE).setMessageFormat("web socket client disconnected"));
    }
  }
  ...
  public void channelRead0(ChannelHandlerContext ctx, Object msg) {
    ...
    if (msg instanceof FullHttpResponse) {
      ...
      if (httpResponse.headers().contains(UPGRADE, WEBSOCKET, true)
        ...
        if (MockServerLogger.isEnabled(WARN) && mockServerLogger != null) {
          mockServerLogger.logEvent(new LogEntry().setLogLevel(WARN).setMessageFormat(message));
        }
        ...
        if (MockServerLogger.isEnabled(TRACE) && mockServerLogger != null) {
          mockServerLogger.logEvent(
              new LogEntry()
                  .setLogLevel(TRACE)
                  .setMessageFormat(
                      "web socket client not required MockServer in same JVM as client"));
        }
        registrationFuture.complete(clientId);
      } else {
        registrationFuture.completeExceptionally(
            new WebSocketException(
                "handshake failure unsupported message received "
      ...
      if (frame instanceof TextWebSocketFrame) {
        ...
        mockServerLogger.logEvent(
            new LogEntry()
                .setLogLevel(WARN)
                .setMessageFormat(
                    "web socket client received an unsupported WebSocketFrame message{}")
                .setArguments(msg));
      }
    } else if (MockServerLogger.isEnabled(WARN) && mockServerLogger != null) {
      mockServerLogger.logEvent(
  ...
  public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
    mockServerLogger.logEvent(
        new LogEntry()
            ...
            .setThrowable(cause));
    final CompletableFuture<String> registrationFuture =
        ctx.channel().attr(REGISTRATION_FUTURE).get();
