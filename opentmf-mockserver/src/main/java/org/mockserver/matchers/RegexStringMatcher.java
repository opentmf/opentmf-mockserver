...
public class RegexStringMatcher extends BodyMatcher<NottableString> {
  ...
  RegexStringMatcher(
      MockServerLogger mockServerLogger, NottableString matcher, boolean controlPlaneMatcher) {
    this.mockServerLogger = mockServerLogger;
    this.controlPlaneMatcher = controlPlaneMatcher;
  ...
  public boolean matches(
    ...
    if (matcher instanceof NottableSchemaString && matched instanceof NottableSchemaString) {
      return controlPlaneMatcher
          && matchesByNottedStrings(mockServerLogger, context, matcher, matched);
      ...
      return controlPlaneMatcher
          && matchesBySchemas(mockServerLogger, context, (NottableSchemaString) matched, matcher);
    } else {
  ...
  private boolean matchesByNottedStrings(
    ...
    if (matcher.isNot() && matched.isNot()) {
      ...
      return (matcher.isNot() || matched.isNot())
          ^ matchesByStrings(mockServerLogger, context, matcher, matched);
    }
  }

  private boolean matchesBySchemas(
      MockServerLogger mockServerLogger,
      MatchDifference context,
      NottableSchemaString schema,
  ...
  private boolean matchesByStrings(
    ...
    if (context != null) {
      context.addDifference(
          mockServerLogger, "string or regex match failed expected:{}found:{}", matcher, matched);
    }
