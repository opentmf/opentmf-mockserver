...
public class HttpRequestToCurlSerializer {
      ...
      if (isNotBlank(request.getFirstHeader(HOST.toString())) || remoteAddress != null) {
        ...
        for (Header header : request.getHeaderList()) {
          for (NottableString headerValue : header.getValues()) {
            curlString
                .append(" -H '")
                .append(header.getName().getValue())
                .append(": ")
                .append(headerValue.getValue())
                .append("'");
            if (header
                .getName()
                .getValue()
                .toLowerCase()
                .contains("Accept-Encoding".toLowerCase())) {
        ...
        if (isNotBlank(request.getBodyAsString())) {
          curlString
              .append(" --data '")
              .append(request.getBodyAsString().replace("'", "\\'"))
              .append("'");
        }
      } else {
  ...
  private boolean hasDefaultMethod(HttpRequest request) {
    return request.getMethod() == null
        || isBlank(request.getMethod().getValue())
        || request.getMethod().getValue().equalsIgnoreCase("GET");
  }
